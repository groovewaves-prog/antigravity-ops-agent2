...
with col_chat:
    ...
    if selected_incident_candidate:
        ...
        if "generated_report" not in st.session_state or st.session_state.generated_report is None:
            ...
            if not api_key:
                st.warning("⚠️ APIキーが設定されていません。サイドバーでAPIキーを入力してください。")
            elif selected_scenario == "正常稼働":
                st.info("ℹ️ 正常稼働中のため、レポート生成は不要です。")
            else:
                # ★ボタンを表示
    ...
    "children": getattr(t_node, "children", []) or [],
        ...
        # selected_incident_candidateがない場合
        st.warning("⚠️ インシデント候補が検出されていません。シナリオを選択するか、テーブルから行を選択してください。")

    # --- B. 自動修復 & チャット ---
    st.markdown("---")
    st.subheader("🤖 Remediation & Chat")

    # ★修正: prob条件を緩和し、selected_incident_candidateが存在すれば表示
    if selected_incident_candidate:
        prob_score = selected_incident_candidate.get("prob", 0)
        ...
        if prob_score > 0.6:
            ...
            <div style="background-color:#e8f5e9;padding:10px;border-radius:5px;border:1px solid #4caf50;color:#2e7d32;margin-bottom:10px;">
                ...
                (リスクスコア: <span style="font-size:1.2em;font-weight:bold;">{prob_score*100:.0f}</span>)
            </div>
            ...
            <div style="background-color:#fff3e0;padding:10px;border-radius:5px;border:1px solid #ff9800;color:#e65100;margin-bottom:10px;">
                <strong>⚠️ 低リスクスコア</strong><br>
                対象: <b>{selected_incident_candidate['id']}</b><br>
                (リスクスコア: <span style="font-size:1.2em;font-weight:bold;">{prob_score*100:.0f}</span> - 通常60以上で自動修復推奨)
            </div>
            """, unsafe_allow_html=True)
